...
Script that allows moving the player from point A to point B in the world.
...
6 - Setting up the interactive grid
-----------------------------------
...
We know that the vertical distance = √3 * inradius.

**Finding the inradius (r):**

We know that ``cell_size.x`` = 2 * inradius.
...
We choose a cylinder with the same top and bottom radius as the mesh, but a different height:

* ``Height`` = **6**
* ``Top_radius`` = inradius = ``cell_size.x`` / 2 = **0.5**
* ``Bottom_radius`` = **0.5**

Dans ``cell_shape_offset.y`` mettre **Height/2** = **3.0**
...
.. image:: ./img/player_pawn_nodes.png
   :alt: player_pawn_nodes.png
   :width: 400px
...
* Click on InteractiveGrid3D → `ShaderMaterial <https://docs.godotengine.org/en/stable/classes/class_shadermaterial.html>`_.
...
* Configure it with its own ``mask``.

* Give it a meaningful name, for example: ``CFL_PULSE``.


* Custom Data Name: **CFL_PULSE**
* Layer Mask : **9**
* Collision Layer : **None**
* Don't use Custom color
